...
const migration = read(
  path.join(root, "supabase/migrations/20260818110000_ap_supplier_statement_foundation.sql"),
);
...
describe("AP supplier statement foundation — architecture finding", () => {
  ...
  it("adds no new table and no new column — only indexes and the one mapping RPC", () => {
    ...
    expect(migration).toContain("CREATE INDEX ap_payments_property_bill");
  });
