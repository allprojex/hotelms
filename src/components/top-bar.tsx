...
export function TopBar() {
  ...
  const { data: properties } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("properties").select("id,name,code").eq("active", true).order("name");
  ...
  useEffect(() => {
    ...
    if (stored) { setActiveId(stored); return; }
      ...
      <div className="flex items-center gap-3">
        {properties && properties.length > 0 && (
          <Select
            ...
            onValueChange={(v) => { setActivePropertyId(v); setActiveId(v); window.dispatchEvent(new Event("iti-property-changed")); }}
          >
