-- AP supplier identity foundation.
--
...
-- one assignment RPC plus the one index the mapping list query needs
-- (ap_bills had no index reaching supplier_id at all before this).
--
-- Scope note: this is PR A of a two-PR split (identity foundation, then
-- statements). The ap_payments(property_id, bill_id) index used by the
-- statement's payment lookup is NOT part of this migration — it belongs to
-- the follow-on statements migration, since nothing in this PR queries
-- ap_payments at all.
